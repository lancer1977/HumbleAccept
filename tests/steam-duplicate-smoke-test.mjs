...
const keyInput = new MockElement({
  ...
  type: "text",
  name: "product_key",
  value: "V23V4-3KKFD-D84FI"
});

const errorDisplay = new MockElement({
  id: "error_display",
  tagName: "span",
  text: "You already own this product. You already consumed this key."
});
...
const documentStub = {
  readyState: "loading",
  body: {
    innerText: "You already own this product. You already consumed this key."
  },
  querySelector(selector) {
    switch (selector) {
...
const sandbox = {
  ...
  window: {
    location: {
      search: "?key=V23V4-3KKFD-D84FI",
      hash: "",
      href: "https://store.steampowered.com/account/registerkey?key=V23V4-3KKFD-D84FI",
  ...
  navigator: {
    clipboard: {
      readText: async () => "V23V4-3KKFD-D84FI",
...
assert.ok(
  ...
  "duplicate message should be reported"
);
assert.equal(
  sentMessages.some((message) => message && message.type === "rateLimitedKey"),
  false,
  "duplicate should not be misclassified as rate limit"
);
assert.deepEqual(copiedTexts, ["V23V4-3KKFD-D84FI"], "duplicate key should be copied to clipboard");

console.log("Steam duplicate smoke test passed");
